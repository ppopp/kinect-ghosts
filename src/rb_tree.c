	...
	rb_tree_node_handle_t* p_next)
{
	...
	if (NULL == last) {
		...
		if (NULL == highKey) {
			...
			while(nil != x) {
				if (1 == (tree->compare(x->key, highKey))) { 
					x = x->left;
				} 
				else {
					next = x;
	...
	json_t** pp_json)
{
	...
	while ((err == NO_ERROR) && (NULL != next)) {
		...
		jsonErr = json_object_set_new(obj, jsonKey, jsonData);

		if (jsonErr) {
			jsonKeyReleaseFunc(jsonKey);
			json_decref(obj);
			return ERR_FAILED_DUMP;
		}
